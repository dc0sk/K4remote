...
**Version:** 0.31 (Draft) · **Date:** 2026-07-20 · **Author:** DC0SK
...
| `FR-UI-HOLD-01` | distinguish a **tap** from a **hold** on controls whose K4 counterpart carries both, using the radio's own threshold of about half a second (D14 p.359), and perform the radio's documented hold function rather than an app-specific one. A press whose start is unknown shall count as a tap. | STK-16 | S | T/D | `is_hold` classifies at the 500 ms boundary and treats an unknown duration as a tap; `agc_tap` never yields AGC-off while `agc_hold` toggles it, so the two gestures differ whenever AGC is on (test); holding AGC on the radio switches it off and tapping cycles slow/fast (demo). |
| `FR-UI-POPUP-01` | open a **settings popup** for a receiver control on a **right-click** of its chip, carrying that control's paired-switch settings (level, mode, on/off) as the K4's own panel does (D14 p.1318 attenuator, p.1368 noise blanker), and dismiss it on a click outside, on `ESC`, or from an explicit dismiss button. The popup shall open **at the control** rather than centred on the window, and shall stay fully within the window. A value the radio quantises shall be snapped to its ladder before being sent. A popup slider shall track the drag from a **local mirror** and shall not re-query the radio mid-drag, so a lagging read-back cannot pull it back to the previous level. | STK-16 | S | T/D | `atten_snap` never yields a level off the radio's 3 dB ladder or above 21 dB, reaches every step, and agrees with `atten_hold`; `popup_origin` tracks the pointer, keeps every popup inside the window from any corner, and pins the top-left when the window is too small; each popup has a distinct title (test); right-clicking each chip opens its panel at that chip, dragging the attenuator holds the dragged level mid-drag, and the three dismissals work (demo). |
