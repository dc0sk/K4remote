...
**Version:** 0.1 (Draft) Â· **Date:** 2026-06-25 Â· **Author:** DC0SK
Trace: owns `FR-`, `NFR-`. Up â [stakeholder-requirements.md](stakeholder-requirements.md);
down â [../concept/architecture.md](../concept/architecture.md) (`ARC`) and
[../test/test-strategy.md](../test/test-strategy.md) (`TC`).

**Legend.** Pri: `M` must (v1) Â· `S` should (v1) Â· `C` could Â· `W2` Phase 2.
Ver(ification): `T` automated test Â· `D` demonstration Â· `I` inspection Â· `A` analysis.
...
## A. Connection & Transport â `FR-CONN`

| ID | Statement (the system shallâ¦) | Up | Pri | Ver | Acceptance criteria |
|---|---|---|---|---|---|
| `FR-CONN-01` | connect to a K4 server by opening a direct TCP socket to host:port (default **9205** plaintext, **9204** TLS-PSK) and completing the authentication handshake (`FR-AUTH-*`). NOTE: a software client does **not** use the `RRT` command â that is the K4-to-K4 "dial a remote server" form (PRG `RRT`); our client *is* the dialing party. | STK-01 | M | T | Given a simulated server, connect opens the socket and runs the auth+init handshake; on success the session reports `Connected`. |
...
| `FR-CONN-05` | apply a configurable connect timeout and fail (not hang) if no handshake response arrives within it. | STK-01 | S | T | With a non-responding server, connect fails within timeout Â±tolerance. |

## B. CAT Protocol Engine â `FR-CAT`

| ID | Statement | Up | Pri | Ver | Acceptance criteria |
|---|---|---|---|---|---|
| `FR-CAT-01` | encode SET commands and decode RESP messages for the supported command set, treating `;` as the frame terminator (PRG Syntax). | STK-02/03 | M | T | Round-trip encodeâdecode of each supported command yields the original typed value. |
...
| `FR-CAT-AI` | enable an Auto-Info mode (`AI`, target `AI5`/`AI4` immediate) on connect and update internal radio state from unsolicited RESP messages (PRG `AI`, NOTE2 per-client). | STK-04 | M | T | After `AI`, a pushed `FAâ¦;` updates VFO-A state with no GET sent. |
...
## B2. Binary Frame & Authentication Layer â `FR-STREAM`
...
| `FR-SES-02` | detect link loss within â¤3 s of the server's 10 s drop threshold being approached (missed PONGs / socket error) and signal it. | STK-01/20 | M | T | Simulated silence â link-loss event within bound. |
...
| `FR-VFO-04` | switch bands â band up/down, **direct band select** by number, band-stacking recall, and transverter bands (PRG `BN`/`BN$`/`BN^`/`XV`). | STK-02 | M | T | Band-up and direct `BN00`â¦`BN10;` encode; band-stack `BN^;` and `XV` encode; RESP updates the band field. |
...
| `FR-VFO-07` | copy/swap the VFOs â AâB, BâA, and swap, for frequency or full state (PRG `AB`). | STK-02 | S | T | `AB0`â¦`AB5;` encode the copy/swap variants. |
...
| `FR-RX-02` | control the RX attenuator (`RA`) and preamp where present, including on/off and level. | STK-03 | M | T | Attenuator 12 dB on emits documented `RAâ¦;`; state reflects RESP. |
...
| `FR-RX-NOTCH-01` | control the **manual notch** (on/off + pitch 150â5000 Hz, `NM`) and **auto-notch** (`NA`) per receiver. | STK-03 | C | T | `set_manual_notch(true,1000)` emits `NM10001;`; `set_auto_notch(true)` emits `NA1;`; RESP reflected. |
...
| `FR-RX-SQL-01` | set and display the main-receiver squelch threshold 0â40 (`SQ`; `$`=sub). | STK-03 | S | T | `set_squelch(22)` emits `SQ022;`, clamps to 40, and reflects the `SQ` RESP. |
...
| `FR-MTR-02` | display the high-resolution S-meter in dBm when available (`SMH`). | STK-04 | S | T | `SMH-073;` shows â73 dBm. |
| `FR-MTR-03` | during transmit, display the TX meters â RF power, SWR, ALC, and CMP (voice) â from the auto-delivered `TM` data. | STK-04 | S | T | `TMâ¦;` populates `tx_alc`/`tx_cmp`/`tx_fwd_w`/`tx_swr_x10`. |
| `FR-MTR-04` | represent meter values on a calibrated scale (bars/dBm/S-units) consistent with the K4 bar graph. | STK-04/11 | S | A/T | Mapping function: bar 00âS0 baseline, 42âtop, monotonic; unit-tested. |

## H. Transmit, Keying & Safety â `FR-TX`
...
| `FR-TX-02` | set transmit power (`PC`). | STK-06 | S | T | Power 50 W emits documented `PCâ¦;`. |
| `FR-TX-CMP-01` | set speech compression 0â30 (`CP`, SSB modes) and reflect the RESP. | STK-06 | C | T | `set_compression(15)` emits `CP015;`, clamps to 30, and reflects the `CP` RESP. |
...
| `FR-TX-SAFE-01` | **fail safe on link loss:** if the link drops while transmitting, immediately cease keying/PTT locally and require re-arming before further TX. | STK-08/13 | M | T | Simulated link loss during TX â local TX state cleared; re-arm required. |
| `FR-TX-SAFE-02` | configure and rely on the radio-side CW fail-safe timeout (`KZF`, 1â10 min) so a stalled stream cannot hold the key down indefinitely. | STK-08 | M | T | `KZF` is set on connect; value configurable. |
...
| `FR-AUD-ENC` | negotiate the audio encode mode (`EM0âEM3`), defaulting to Opus (`EM3`) for WAN and allowing raw PCM on LAN (PRG `EM`; CON-06). | STK-05/14 | M | T | Default emits `EM3;`; LAN profile can select `EM0/1`. |
| `FR-AUD-02` | buffer received audio with a jitter buffer to tolerate network jitter, with a bounded added latency. | STK-05 | M | T | Reordered/late frames within the buffer window play in order; latency â¤ target. |
...
| `FR-PAN-01` | **[Phase 2]** decode the PAN packet (`0x02`): receiver, center freq (i64 LE Hz), sample rate (i32 LE; span = Ã1000 Hz), noise floor (i32 LE Ã·10 dB), and bins as 1 byte each where **dBm = byte â 146** (`R-EXT-01`). | STK-09 | W2 | T | Decoder produces the expected dBm array + metadata from sample fixtures; MiniPAN (`0x03`) likewise. |
...
| `FR-UI-08` | provide a switchable main-window **view mode** â single VFO A, single VFO B, or dual (A+B) â that reflows the header + panadapter layout, mirroring the K4 `PAN=A/B/A+B` selection (`R-EXT-02`, `ui-design.md`). | STK-11 | M | T | `ViewMode` cycles AâBâdualâA; each mode reports which receiver pane(s) are shown. |
| `FR-UI-09` | render operating frequencies with the K4's **dot-grouped** formatting to kHz (e.g. `14.070.000`). | STK-11 | M | T | Formatter maps Hz â grouped string across band edges and sub-kHz values. |
| `FR-UI-10` | apply **semantic colour roles** to operating state â transmit (amber), VFO A/main (blue), VFO B/sub & active (green), RX values (white), caution (yellow), inactive (dim) â so TX/RX and A/B are unmistakable (`FR-UI-06`). | STK-11/08 | M | T | Role selector returns the correct role for TX/RX, A/B, active/inactive inputs. |
| `FR-UI-11` | present operating controls as **two-line state buttons** (function label + live value derived from the radio state), e.g. `ATT`/`Off`, `AGC`/`Slow`, `BW`/`2.80`. | STK-11 | S | T | Stateâ(label,value) derivation matches the radio state for representative controls. |
...
| `FR-UI-15` | style the client after the references (`R-EXT-02`): a **dark layered theme** whose background/panel/control surfaces step up strictly in luminance, and a **proportional S-meter bar** on the K4 face (S1 â â121 dBm â¦ S9+60 â â13 dBm, S9 = â73 dBm), green with a caution colour â¥ S9. | STK-11 | S | T/D | Shade palette is strictly luminance-ordered and `s_meter_fraction` maps the face endpoints/S9 correctly, clamped (test); themed panels/buttons/meters render per `ui-design.md` (demo). |
...
| `FR-UI-17` | offer a **theme selector** cycling **Dark â Light â Contrast â System**, applied live to the whole UI; `System` follows the OS light/dark preference. Each theme resolves the surface-shade and semantic-role palettes (`FR-UI-10/15`). | STK-11 | C | T/D | `ThemeMode` cycles the four modes with distinct labels and resolves to a concrete palette (`System` per the detected OS preference) (test); each theme renders coherently (demo). |
...
| `FR-UI-22` | show a **phase-coloured connection indicator** in the header â a dot + label that is green when connected, amber while connecting, and grey when disconnected. | STK-01/11 | C | T/D | `conn_status(phase)` returns the correct (label, colour role) per phase (test); the header dot changes colour across a connect cycle (demo). |
...
## N. Radio configuration commands (screen support) â `FR-EQ / FR-KEY / FR-AUD-CFG / FR-ANT / FR-MENU`
...
| ID | Statement (the system shallâ¦) | Up | Pri | Ver | Acceptance criteria |
|---|---|---|---|---|---|
| `FR-EQ-01` | control the **8-band graphic equalizer** for RX (per receiver) and TX â set each 100â3200 Hz band's gain (â16..+16 dB) and flatten (PRG `RE`/`TE`/`REF`). | STK-03/06 | S | T | `RE`/`TE` encode 8 signed 3-char band fields within Â±16; `REF;` flattens. |
| `FR-KEY-01` | configure the **CW keyer**: weight, paddle normal/reverse, iambic mode A/B, and speed (PRG `KP`/`KS`). | STK-07 | S | T | `KP`/`KS` encode within range (weight 90â125, speed 8â100 WPM). |
| `FR-KEY-02` | set the **CW sidetone/pitch** 250â950 Hz (`CW`, Ã10). | STK-07 | C | T | `set_cw_pitch(600)` emits `CW60;`, clamps to 250â950 Hz. |
...
| `FR-ANT-01` | select the **transmit antenna** (ANT1â3) and per-receiver **RX antenna** (PRG `AN`/`AR`). | STK-02 | C | T | `AN`/`AR` encode within range. |
| `FR-MENU-01` | access the radio's **configuration menu** by item id â open, query definition, and set a menu parameter (PRG `MO`/`MEDF`/`ME`). | STK-11 | C | T | `MO`/`MEDF`/`ME` encode the 4-digit id (and value for set). |
| `FR-SW-01` | **emulate front-panel switch** tap/hold by code (PRG `SW`) to reach functions that have no dedicated CAT command â notably quick memories M1âM4 (recall/store) and PF1âPF4 (the memory-channel `MC` command being pending in D12). | STK-02/11 | C | T | `switch(code)` encodes `SW<code>;`; the quick-memory/PF/switch tables carry the correct codes. |
...
| `FR-TX-MSG-01` | **send CW/DATA text messages** for transmission (PRG `KY`, â¤60 chars). | STK-07 | C | T | `send_text(text)` encodes `KY <text>;` and truncates to 60 chars. |
| `FR-TXT-01` | enable **text decode** for the active receiver (`TD` mode/threshold/lines) and display the decoded receive text polled from the buffer (`TB`, `s` field, `;`-safe). | STK-07 | C | T | `set_text_decode(2,0,3)` emits `TD203;`; a `TBâ¦` RESP appends its text to `decode_text`, preserving embedded `;`. |
| `FR-PWR-01` | provide **remote power control** â **power off** (PRG `PS0`) and **restart** (`PS8`) â with the power-off action **guarded** against accidental activation (explicit confirm). Powering the radio **on** is not possible via CAT (the interface is unpowered when off, per D12). | STK-11 | C | T/D | `set_power(n)` encodes `PS<n>;` (0/8/88); the UI power-off requires a two-step confirm (demo). |

---

## Non-Functional Requirements â `NFR`

| ID | Statement | Up | Pri | Ver | Acceptance criteria |
|---|---|---|---|---|---|
| `NFR-PERF-01` | Control round-trip latency (UI action â command sent â RESP applied) shall be â¤150 ms on a LAN, excluding network RTT. | STK-01/02 | M | T/A | Measured engine-side latency budget met in benchmark harness. |
| `NFR-PERF-CW` | Local CW keying jitter introduced by the client (paddle event â `KZ` emission) shall be â¤10 ms typical. | STK-07 | M | T | Timing test: emission delay distribution within bound. |
| `NFR-PERF-AUDIO` | End-to-end added audio latency from client buffering shall be â¤120 ms (target; tunable jitter buffer). | STK-05 | M | A/D | Measured/configured buffer within target; documented trade-off. |
...
| `NFR-REL-FAILSAFE` | On any link loss while transmitting, the client shall reach a non-transmitting safe state within â¤1 s. | STK-08 | M | T | Fault-injection test meets the time bound. |
...
| `NFR-TEST-01` | Every `M` and `S` functional/non-functional requirement shall be covered by â¥1 automated test referencing its ID (rule R3/R4). | STK-15 | M | I | Traceability gate `xtask trace` green. |
...
- `OP-4` Decide CW source: physical paddle via serial/USB at the client, on-screen, or keyboard â affects `FR-TX-CW-01` input layer. *(QK4 supports hardware keyers/K-Pod; out of our v1 scope.)*
...
| 2026-07-02 | 0.4 | DC0SK | Added FR-UI-15 (reference-faithful dark layered theme + proportional S-meter bar) â visual-identity pass per updated ADR-15 direction. |
...
| 2026-07-02 | 0.8 | DC0SK | Phase-0 config-screen commands: added Â§N (FR-EQ-01, FR-KEY-01, FR-AUD-CFG-01, FR-ANT-01, FR-MENU-01) + FR-VFO-07 (VFO copy/swap); extended FR-VFO-04 (direct band/stack/XVTR) and FR-PAN-CTL-01 (full `#` display family). CAT encoders + tests added to `k4-protocol` (`docs/concept/k4-screens.md` Â§3.2). |
| 2026-07-02 | 0.9 | DC0SK | Added FR-SW-01 (front-panel switch emulation `SW`) â enables quick memories (M1âM4) and PF1âPF4 remotely, since the `MC` memory-channel command is pending in D12. |
...
| 2026-07-03 | 0.11 | DC0SK | Added FR-UI-20 (config screens seed from the radio on connect â read-back). FR-AUTH-02 (TLS-PSK, port 9204) now **implemented** and verified live against a real K4 (`connect_tls`): learned the exact PSK scheme (identity empty, key = raw password bytes, TLS 1.2 `PSK-AES256-CBC-SHA384`) and fixed a handshake-timeout + OpenSSL-security-level bug. Keychain I/O moved off the UI thread (FR-UI-07 hardening). |
| 2026-07-03 | 0.12 | DC0SK | UI polish: extended FR-UI-18 (About now shows version + donate link + openable license/project/donate links); added FR-UI-21 (landscape default window) and FR-UI-22 (phase-coloured connection indicator). Also: TX box CLR sized to match SPLIT/RIT/XIT (2Ã2 grid). |
...
| 2026-07-06 | 0.14 | DC0SK | Added FR-PWR-01 (remote power off `PS0` / restart `PS8`, guarded power-off). No power-on via CAT (D12). Control on the Fn â SWITCHES tab. |
