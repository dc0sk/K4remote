...
| `FR-SPOT-09` | Spot nameplates (source failure isolation; Ver `T/D`, priority `S`): built and tested for the **telnet** sources (`fr_spot_09_*`, which is why the coverage table shows it traced); the **PSK Reporter** source does not exist yet (`FR-SPOT-05`), so the requirement is not met for every source. Keep this waiver until it is. |
